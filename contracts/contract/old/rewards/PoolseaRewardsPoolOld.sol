...
import "../../../interface/old/PoolseaRewardsPoolInterfaceOld.sol";
...
contract PoolseaRewardsPoolOld is PoolseaBase, PoolseaRewardsPoolInterfaceOld {
    ...
    * @return uint256 Returns rpl balance of poolsea rewards contract
    */
    function getRPLBalance() override public view returns(uint256) {
        ...
        return poolseaVault.balanceOfToken("poolseaRewardsPool", IERC20(getContractAddress("poolseaTokenRPL")));
    }
    ...
    function getPendingETHRewards() override public view returns (uint256) {
        address poolseaSmoothingPoolAddress = getContractAddress("poolseaSmoothingPool");
        return poolseaSmoothingPoolAddress.balance;
    }
    ...
    */
    ...
    function submitRewardSnapshot(RewardSubmission calldata _submission) override external onlyLatestContract("poolseaRewardsPool", address(this)) onlyTrustedNode(msg.sender) {
        // Get contracts
        ...
        require(poolseaDAOProtocolSettingsNetwork.getSubmitRewardsEnabled(), "Submitting rewards is currently disabled");
        // Validate inputs
    ...
    function executeRewardSnapshot(RewardSubmission calldata _submission) override external onlyLatestContract("poolseaRewardsPool", address(this)) {
    ...
    function _executeRewardSnapshot(RewardSubmission calldata _submission) private {
        ...
        if (_submission.treasuryRPL > 0) {
            poolseaVault.transferToken("poolseaClaimDAO", rplContract, _submission.treasuryRPL);
        }
        // Get the smoothing pool instance
        PoolseaSmoothingPoolInterface poolseaSmoothingPool = PoolseaSmoothingPoolInterface(getContractAddress("poolseaSmoothingPool"));
        // Send deposit pool user's ETH
        if (_submission.userETH > 0) {
            address poolseaTokenRETHAddress = getContractAddress("poolseaTokenRETH");
            poolseaSmoothingPool.withdrawEther(poolseaTokenRETHAddress, _submission.userETH);
        }
        ...
        for (uint i = 0; i < _submission.nodeRPL.length; i++) {
            ...
            PoolseaRewardsRelayInterface relay;
            { // Scope to prevent stack too deep
                ...
                relay = PoolseaRewardsRelayInterface(networkRelayAddress);
            }
            // Transfer rewards
            if (rewardsRPL > 0) {
                // RPL rewards are withdrawn from the vault
                poolseaVault.withdrawToken(address(relay), rplContract, rewardsRPL);
            }
            if (rewardsETH > 0) {
                // ETH rewards are withdrawn from the smoothing pool
                poolseaSmoothingPool.withdrawEther(address(relay), rewardsETH);
            }
