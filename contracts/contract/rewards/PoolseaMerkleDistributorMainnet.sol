...
contract PoolseaMerkleDistributorMainnet is PoolseaBase, PoolseaRewardsRelayInterface, PoolseaVaultWithdrawerInterface {
    ...
    // Immutables
    bytes32 immutable poolseaVaultKey;
    bytes32 immutable poolseaTokenRPLKey;
    ...
    constructor(PoolseaStorageInterface _poolseaStorageAddress) PoolseaBase(_poolseaStorageAddress) {
        ...
        version = 1;
        // Precompute keys
        poolseaVaultKey = keccak256(abi.encodePacked("contract.address", "poolseaVault"));
        poolseaTokenRPLKey = keccak256(abi.encodePacked("contract.address", "poolseaTokenRPL"));
    ...
    // Called by PoolseaRewardsPool to include a snapshot into this distributor
    function relayRewards(uint256 _rewardIndex, bytes32 _root, uint256 _rewardsRPL, uint256 _rewardsETH) external override onlyLatestContract("poolseaMerkleDistributorMainnet", address(this)) onlyLatestContract("poolseaRewardsPool", msg.sender) {
        ...
        PoolseaVaultInterface poolseaVault = PoolseaVaultInterface(getAddress(poolseaVaultKey));
        if (_rewardsETH > 0) {
            poolseaVault.depositEther{value: _rewardsETH}();
        }
        if (_rewardsRPL > 0) {
            IERC20 poolseaTokenRPL = IERC20(getAddress(poolseaTokenRPLKey));
            poolseaTokenRPL.approve(address(poolseaVault), _rewardsRPL);
            poolseaVault.depositToken("poolseaMerkleDistributorMainnet", poolseaTokenRPL, _rewardsRPL);
        }
    }
    ...
    function claimAndStake(address _nodeAddress, uint256[] calldata _rewardIndex, uint256[] calldata _amountRPL, uint256[] calldata _amountETH, bytes32[][] calldata _merkleProof, uint256 _stakeAmount) public override {
        // Get contracts
        PoolseaVaultInterface poolseaVault = PoolseaVaultInterface(getAddress(poolseaVaultKey));
        address poolseaTokenRPLAddress = getAddress(poolseaTokenRPLKey);
        // Verify claims
        _claim(_rewardIndex, _nodeAddress, _amountRPL, _amountETH, _merkleProof);
        {
            ...
            if (remaining > 0) {
                poolseaVault.withdrawToken(withdrawalAddress, IERC20(poolseaTokenRPLAddress), remaining);
            }
            // Distribute ETH
            if (totalAmountETH > 0) {
                poolseaVault.withdrawEther(totalAmountETH);
        ...
        if (_stakeAmount > 0) {
            ...
            poolseaNodeStaking.stakeRPLFor(_nodeAddress, _stakeAmount);
        }
        // Emit event
    ...
    // Allow receiving ETH from PoolseaVault, no action required
